...
*/
	function settings(cmd, p){
		...
		StringToFile("hrc/manacat_tank/ai_settings.txt", set);
	}

	function OnGameEvent_round_start_post_nav(params){
		local vars = FileToString("hrc/manacat_tank/ai_settings.txt");
		local load = true;
		if(vars != null){
			vars = split(vars, "|");
		...
		if(vars == null || !load){
			...
			StringToFile("hrc/manacat_tank/ai_settings.txt", vars);
			vars = split(vars, "|");
		}
