...
void CPredictionCopy::CopyFields( int chain_count, datamap_t *pRootMap, typedescription_t *pFields, int fieldCount )
{
	...
	for ( i = 0; i < fieldCount; i++ )
	{
		...
		if ( m_pCurrentField->fieldType != FIELD_EMBEDDED )
		{
			...
			// For MPC_NON_NETWORKED_ONLYs skip any fields that are present in the network send tables
			if ( m_nType == MPC_NON_NETWORKED_ONLY && ( flags & FTYPEDESC_INSENDTABLE ) )
				continue;

			// For MPC_NETWORKED_ONLYs skip any fields that are not present in the network send tables
			if ( m_nType == MPC_NETWORKED_ONLY && !( flags & FTYPEDESC_INSENDTABLE ) )
				continue;
		}
		...
		case FIELD_EMBEDDED:
			{
				...
				m_pSrc = pInputData;
				if ( ( flags & FTYPEDESC_PTR ) && (m_nSrcOffsetIndex == MPC_DATA_NORMAL) )
				{
					m_pSrc = *((void**)m_pSrc);
				}

				m_pDest = pOutputData;
				if ( ( flags & FTYPEDESC_PTR ) && (m_nDestOffsetIndex == MPC_DATA_NORMAL) )
				{
...
void CPredictionCopyTester::RunTests( void )
{
...

	{
		...
		CPredictionCopy tester( MPC_NON_NETWORKED_ONLY, foo1, false, foo3, false, true );
		int diff_count = 0;
...

	{
		...
		CPredictionCopy tester( MPC_NON_NETWORKED_ONLY, foo1, false, foo2, false, true, false );
		int diff_count = 0;
...

	{
		...
		CPredictionCopy tester( MPC_NON_NETWORKED_ONLY, foo1, false, foo2, false, true );
		int diff_count = 0;
		diff_count = tester.TransferData( foo2->GetPredDescMap(), foo1->GetPredDescMap()->dataDesc, foo1->GetPredDescMap()->dataNumFields );
